...
This record distinguishes checks that were actually run from behavior that remains untested. Passing package, archive, plugin-listing, or documentation checks does not establish cultivation-field fitness, legal correctness, live-host behavior, or accessibility conformance.
...
The operating kernel is unchanged from v0.1.4. The reviewed three-case context-only safety and scope smoke retained from v0.1.2 remains inherited evidence under its original runtime and prompts. It was not rerun and is not new v0.1.5 behavioral evidence.
