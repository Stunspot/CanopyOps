...
def package(source: Path, output: Path, top_level: str = "") -> int:
    ...
    with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        for path in sorted(files, key=lambda p: p.relative_to(source).as_posix().lower()):
            ...
            archive_name = f"{top_level.strip('/')}/{relative}" if top_level else relative
...
def main() -> int:
    ...
    count = package(args.source.resolve(), args.output.resolve(), args.top_level)
