# Install CanopyOps in Codex

## Prerequisites

- An extracted `CanopyOps-v0.1.6.zip` release.
...
2. Confirm the payload contains [plugin.json](../codex/canopyops/.codex-plugin/plugin.json) and a `codex/canopyops/skills/` directory.
3. In Codex's supported local-plugin import flow, select the complete `codex/canopyops/` directory. If the host instead uses a configured plugin source directory, copy that whole directory there unchanged; do not copy individual skill files out of it.
4. Let Codex reload plugins, then open a fresh task so discovery is tested without stale task state.
5. Confirm `CanopyOps` and its expected handles are listed by the host.
...
2. If verification passes but the plugin is absent, confirm the host supports local plugins and that the selected directory is `codex/canopyops/`, not its parent or `skills/` child.
