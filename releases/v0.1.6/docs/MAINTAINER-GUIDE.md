...
1. Confirm the plan names `canopyops` version `0.1.6` and the intended repository, visibility, handles, descriptions, starter prompt, and backup filename.
...
5. Run the official Codex plugin validator against `families/canopyops/v0.1.6/codex/canopyops/`.
6. Run the installed documentation accessibility linter across `families/canopyops/v0.1.6/docs/`.
7. Require an independent review of archive safety, byte parity, description custody, and customer working paths.
8. Promote to the [custody repository](https://github.com/Stunspot/CanopyOps) only after every gate passes. Re-clone the remote and compare hashes before crediting GitHub custody.

## Family maintenance responsibilities

## canopyops

- workflow/reference/template routing
- calculation scripts and transparent manual fallback
- jurisdiction/label/SOP source requirements
...
- Detached `receipt.json` and `CanopyOps-v0.1.6.zip.sha256`: canonical and backup archive custody.
