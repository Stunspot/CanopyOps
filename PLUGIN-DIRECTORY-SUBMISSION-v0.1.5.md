...
> **Historical custody record:** This packet records the v0.1.5 skills-only draft state observed on July 20, 2026. It does not assert the current portal state, review submission, approval, publication, or discoverability. Read [`RELEASE-STATUS.md`](RELEASE-STATUS.md) for the current repository claim boundary.

This packet maps the released CanopyOps v0.1.5 plugin to the OpenAI Plugins Directory form used at the evidence cutoff and records the created **Skills only** draft. Draft creation and publisher-identity selection were observed; owner policy attestations, submission for review, approval, publication, and discoverability remained separate states.

OpenAI’s current general explanation of Plugins in ChatGPT and Codex is available at https://help.openai.com/en/articles/20001256-plugins-in-codex. Current platform documentation does not retroactively prove the state of this CanopyOps draft.
...
The draft was created from a preflight ZIP with the same member names. Seventy-five members were byte-identical and the Ella Greenfield persona was identical after CRLF/LF normalization. Its ZIP container metadata was not deterministic. The live portal accepted that shape at the evidence cutoff.
...
- Observed draft state on July 20, 2026: listing, three prompts, one skill, and capability tags saved; the skill passed automated scanning; policy attestations unchecked; review submission not executed.

The identifiers above are custody data. Their presence does not prove that the draft still exists, remains unchanged, or is visible to another account.

## Info

- Plugin name: **CanopyOps**
- Portal subtitle: **Cannabis crop operations** (24 characters; the observed form capped this field at 30)
...
- Developer identity: **Business — Collaborative Dynamics Inc** selected in the portal; listing developer name **Collaborative Dynamics Inc**
...
- **Expected result:** Bounded crop plan with assumptions, decision ranges, monitoring cadence, exception triggers, record fields, and explicit items requiring local SOP or licensed-professional confirmation.
- **Fixture:** Fictional licensed facility profile, room dimensions, cultivar notes, equipment limits, labor availability, and local operating constraints.
...
- **Expected result:** Evidence table, ranked differential, immediate containment if warranted, measurement plan, reversible next actions, stop conditions, and an incident record.
- **Fixture:** Synthetic seven-day log containing irrigation volume, drain percentage, input and runoff EC/pH, substrate water content, temperature, humidity, and observations.
...
- **Expected result:** Incident timeline, evidence and unknowns, ranked risks, immediate reversible actions, inspection plan, escalation thresholds, and recovery checks.
- **Fixture:** Fictional licensed-room log with an overnight humidity spike, HVAC status, canopy observations, and no confirmed pathogen finding.
...
- **Expected result:** Readiness criteria table, evidence gaps, batch-level recommendation, hold or recheck conditions, record entry, and explicit human decision owner.
- **Fixture:** Fictional batch record with staged observations, sampling notes, quality targets, labor calendar, and local SOP references.
...
- **Expected result:** Input table, normalized units, calculation, uncertainty range, proposed shot strategy, monitoring checkpoints, exceptions, and a cultivation-decision record.
...
- **Why:** The product scope is explicitly limited to legal cultivation and cannot treat an unlawful operation as an ordinary facility-planning task.
...
- **Why:** Off-label pesticide instructions can create legal, worker-safety, crop-safety, and consumer-safety harm.
...
- **Why:** CanopyOps provides cultivation decision support, not medical diagnosis or occupational-health clearance.
...
## Release notes supplied for reviewer custody

Initial public submission of the free CanopyOps skills-only plugin. CanopyOps provides evidence-bounded crop planning, diagnostics, calculations, incident workups, harvest review, and auditable operating records for legal cannabis cultivation. Version 0.1.5 adds the production listing package and legible CanopyOps canopy-grid icon, with validated Codex plugin, standalone Augment, standalone skill, and Claude distributions. It includes no account, telemetry, hosted service, connector, or MCP server.

## Accountable-owner gate

Before selecting **Submit for Review**, the publisher must personally confirm the verified developer or business identity, organization and Apps Management authority, country availability, public URLs, and every policy attestation.

Submission begins OpenAI review. Approval, publisher release, public availability, and discoverability are later observable states.

The form observed on July 20, 2026 did not expose the documentation page’s separate Testing, Global, or release-notes panels. The exact five positive cases, three negative cases, availability decision, and release notes remain preserved above for reviewer follow-up or a later portal revision; their repository presence does not mean they were transmitted in the draft.
